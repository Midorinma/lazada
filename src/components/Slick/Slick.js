...
function SlickProducts({ listImg = [] }) {
    const [state, setState] = useState({});
    const images = [];
    listImg.forEach((item) => item.forEach((i) => images.push(i)));
    const [hoveredImage, setHoveredImage] = useState(images[0]); // State để lưu link ảnh khi hover
    const imageZoomRef = useRef();

    const props = {
        width: 330,
        height: 330,
        zoomStyle: 'height:500px; width:830px; z-index:1',
        offset: { vertical: 0, horizontal: 10 },
        zoomLensStyle:
            'object-fit: cover;cursor: zoom-in;opacity:0.1;background-color: blue;',
    };
    ...
    useEffect(() => {
        const imgRef = imageZoomRef.current;
        const firstChild = imgRef.container.firstChild;
        imgRef.container.style.display = 'flex';
        if (firstChild) {
            imgRef.container.removeChild(firstChild);
        }
